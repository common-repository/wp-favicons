 ...
 * see: http://www.phpclasses.org/package/2369-PHP-Extract-graphics-from-ico-files-into-PNG-images.html
 **/
...
class Ico {
			...
			switch ($this->formats[$i]['BitCount']) {
				...
				case 4:
					...
					for ($j = 0; $j < $this->formats[$i]['ColorCount']; $j++) {
						$this->formats[$i]['colors'][] = array(
						// File class.ico.php lines 131 and 133, 'blue' and 'red' should be permuted.
						// From: http://www.phpclasses.org/discuss/package/2369/thread/1/
	...
	function &GetIcon($index) {
		// replaced with see: http://www.tom-reitz.com/2009/02/17/php-ico-to-png-conversion/
		if (is_null($index)) {
		...
		if (in_array($this->formats[$index]['BitCount'], array(1, 4, 8, 24))) {
			if ($this->formats[$index]['BitCount'] != 24) {
				...
				for ($i = 0; $i < $this->formats[$index]['ColorCount']; $i++) {
				    //with update from {@link http://www.tom-reitz.com/2009/02/17/php-ico-to-png-conversion/}
					// see: http://www.tom-reitz.com/2009/02/17/php-ico-to-png-conversion/
			...
			for ($i = 0; $i < $total_bytes; $i++) {
				// @todo: Notice: Uninitialized string offset: 64 in C:\xampp\htdocs\wp2\wp-content\plugins\wp-favicons\plugins\filters\inc\class.ico.php on line 296
				// e.g. from http://www.slatch.com/ 's favicon
